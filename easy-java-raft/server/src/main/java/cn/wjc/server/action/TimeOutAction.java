...
public class TimeOutAction implements Runnable {
    ...
    public void run() {
        ...
        if (current - node.preElectionTime < node.electionTime) {
            // raft随机超时时间
            node.electionTime = node.electionTime + ThreadLocalRandom.current().nextInt(1500);

            node.state = State.CANDIDATE;
            node.currentTerm = node.currentTerm + 1;
            node.preElectionTime = System.currentTimeMillis();
            log.debug("定时器超时，进入新一轮的选举状态，当前Trem:" + node.currentTerm);
            ...
            for (Peer peer : peers) {
                ...
                try {
                    client.send(peer.getAddr(), request);
                } catch (Exception e) {
